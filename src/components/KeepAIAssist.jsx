...
export default function KeepAIAssist({ keepId, content, intentType, accessToken, userLanguage = 'en' }) {
  ...
  async function askAI(selectedAction) {
    ...
    try {
      const { data: res, error: resErr } = await apiPost('/api/keep-assist', { keepId, content, intent_type: intentType, action: selectedAction, userLanguage }, accessToken);
      const data = res;
      if (data.error) {
        if (res.status === 401) setErr('session_expired');
        else if (res.status === 429 || data.upgrade) setErr('limit:' + (data.error || 'Daily limit reached.'));
        else setErr(data.error || 'AI unavailable — try again shortly.');
      }
  ...
  return (
    <div style={{ marginTop: '10px', background: '#0d0d1a', border: '1px solid #6366f130', borderRadius: 10, padding: '12px', fontSize: '12px' }}>
      ...
      {err && (
        <div style={{ fontSize: '11px', padding: '10px 12px', background: 'rgba(239,68,68,0.06)', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.18)' }}>
          {err === 'session_expired' ? (
            <span style={{ color: '#f87171' }}>⚠️ Session expired — please refresh the page.</span>
          ) : err.startsWith('limit:') ? (
            <div>
              <div style={{ color: '#f87171', marginBottom: 6 }}>⚠️ {err.slice(6)}</div>
              <a href="/subscription" style={{ color: '#818cf8', fontWeight: 700, textDecoration: 'none', fontSize: '11px' }}>
                ↗ Upgrade for more AI calls
              </a>
            </div>
          ) : (
            <span style={{ color: '#f87171' }}>⚠️ {err}</span>
          )}
        </div>
      )}
