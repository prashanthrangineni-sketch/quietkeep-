...
export default function KidsPage() {
  ...
  async function uploadFile() {
    ...
    setUploadPct(70);
    const { data: pubData } = supabase.storage.from('kids-content').getPublicUrl(path);
    const publicUrl = pubData?.publicUrl || '';
    const { data: signData, error: signErr } = await supabase.storage.from('kids-content').createSignedUrl(path, 60*60*24*365);
    if (signErr) { setErr('Upload succeeded but URL generation failed: ' + signErr.message); setUploading(false); return; }
    const signedUrl = signData?.signedUrl || publicUrl;
    setUploadPct(90);
      ...
      <div style={{ maxWidth:480, margin:'0 auto', padding:'6rem 16px 6rem' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
          <div>
            ...
            <div style={{ fontSize:12, color:'var(--text-subtle)' }}>Profiles · files · memories</div>
          </div>
          ...
          <div style={{ background:'rgba(99,102,241,0.08)', border:'1px solid rgba(99,102,241,0.25)', borderRadius:16, padding:20, marginBottom:24 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12 }}>
              <div>
                ...
                <input style={{ width:'100%', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, color:'var(--text)', padding:'10px 12px', fontSize:14, outline:'none', boxSizing:'border-box', fontFamily:'inherit' }}
              ...
              <div>
                ...
                <input type="date" style={{ width:'100%', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, color:'var(--text)', padding:'10px 12px', fontSize:14, outline:'none', boxSizing:'border-box', fontFamily:'inherit' }}
        ...
        {kids.length === 0 && !showForm ? (
          <div style={{ textAlign:'center', padding:'60px 20px' }}>
            ...
            <div style={{ fontSize:16, fontWeight:700, color:'var(--text)', marginBottom:8 }}>No child profiles yet</div>
            <div style={{ fontSize:13, color:'var(--text-subtle)' }}>Tap + Add Child to create the first profile</div>
          </div>
        ) : (
        ...
        {activeKid && (
          <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid var(--surface)', borderRadius:16, padding:20, marginBottom:24 }}>
            ...
            {showUpload && (
              <div style={{ background:'var(--surface)', borderRadius:12, padding:16, marginBottom:16 }}>
                ...
                <input type="file" accept="image/*,application/pdf,.doc,.docx"
                  ...
                  style={{ fontSize:12, color:'var(--text-muted)', marginBottom:8, width:'100%' }} />
                ...
                <button onClick={uploadFile} disabled={uploading || !uFile}
            ...
            {/* Content list */}
            {loadingContent ? (
              <div style={{ textAlign:'center', padding:20, color:'var(--text-muted)', fontSize:13 }}>Loading files…</div>
            ) : content.length === 0 ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {content.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{ display:'flex', alignItems:'center', gap:12, background:'var(--surface)', borderRadius:10, padding:'10px 12px' }}>
                      <div style={{ fontSize:22, flexShrink:0 }}>{ct.label.split(' ')[0]}</div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize:13, fontWeight:600, color:'var(--text)', marginBottom:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item.title || item.file_name}</div>
                        <div style={{ fontSize:11, color:'var(--text-subtle)' }}>{ct.label} · {item.file_size ? fmt(item.file_size) : ''}</div>
                      </div>
                      {item.file_url && (
