...
export default function LoginPage() {
  ...
  async function sendOtp() {
    ...
    const beta = BETA_EMAILS.includes(emailNorm);
    setIsBeta(beta);
    if (beta) {
      // Beta: skip email — just show OTP boxes, user types 00000000
      setStep('otp');
      setTimeout(() => refs.current[0]?.focus(), 120);
      return;
    }
    // Real users: send actual OTP
  ...
  async function verifyOtp(digits: string[]) {
    ...
    if (isBeta) {
      ...
      // Use plain createClient (no PKCE) for password sign-in
  ...
  if (step === 'email') return (
    <div style={wrap}>
      <div style={card}>
        ...
        <div style={{ fontSize: '13px', color: '#666', marginBottom: '28px' }}>Enter your email to sign in.</div>
  ...
  return (
    <div style={wrap}>
      <div style={{ ...card, textAlign: 'center' }}>
        ...
        <div style={{ fontSize: '18px', fontWeight: 700, marginBottom: '6px' }}>{isBeta ? 'Beta Access' : 'Enter your code'}</div>
        <div style={{ fontSize: '13px', color: '#888', marginBottom: '24px', lineHeight: 1.6 }}>
          {isBeta
            ...
            : <>{OTP_LEN}-digit code sent to <strong style={{ color: '#c4b5fd' }}>{email}</strong></>
          }
        </div>
        ...
        {!isBeta && (
          <div style={{ marginTop: '16px', fontSize: '13px', color: '#555' }}>
            {countdown > 0
              ...
              : <button onClick={() => { setError(''); setOtp(Array(OTP_LEN).fill('')); sendOtp(); }} style={{ background: 'none', border: 'none', color: '#818cf8', cursor: 'pointer', fontSize: '13px', fontFamily: 'inherit', textDecoration: 'underline' }}>Resend code</button>
            }
          </div>
        )}
        <div style={{ marginTop: '8px' }}>
          <button onClick={() => { setStep('email'); setOtp(Array(OTP_LEN).fill('')); setError(''); setIsBeta(false); }} style={{ background: 'none', border: 'none', color: '#444', cursor: 'pointer', fontSize: '12px', fontFamily: 'inherit' }}>← Use different email</button>
        </div>
      </div>
    </div>
  );
