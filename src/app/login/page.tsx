...
const postAuthPath = APP_TYPE === 'business' ? '/b/dashboard' : '/dashboard';
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID) {
      console.error('Google client ID (NEXT_PUBLIC_GOOGLE_CLIENT_ID) is not configured. Google Sign-In button is hidden.');
    }
  }, []);
  ...
  async function signUp() {
    ...
    const { data, error: signUpErr } = await getClient().auth.signUp({
      ...
      password,
      options: {
        emailRedirectTo: `${window.location.origin}/auth/callback?next=${postAuthPath}`,
      },
    });
  ...
  async function sendReset() {
    ...
    const { error: resetErr } = await getClient().auth.resetPasswordForEmail(
      ...
      { redirectTo: `${window.location.origin}/auth/confirm?next=${postAuthPath}` }
    );
    ...
    try {
      ...
      if (!isNativeApp) {
        const { error: oauthErr } = await getClient().auth.signInWithOAuth({
          provider: 'google',
          options: { redirectTo: `${window.location.origin}/auth/callback?next=${postAuthPath}` },
        });
        if (oauthErr) {
      ...
      const idToken = user?.authentication?.idToken || (user as any)?.idToken;
      if (idToken) {
