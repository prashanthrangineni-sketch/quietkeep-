...
function parseDateTime(text) {
  ...
  if (timeMatch) {
    ...
    hours = 0; minutes = 0;
  } else if (/\bnoon\b|\bmidday\b/.test(t)) {
    hours = 12; minutes = 0;
...
export default function Dashboard() {
  ...
  async function updateState(id, state) {
    ...
    await supabase.from('audit_log').insert([{
      user_id: user.id,
      action: 'intent_state_changed',
      service: 'dashboard',
      details: { intent_id: id, new_state: state },
    }]);
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            ...
            {voiceSupported && (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: listening ? 'rgba(239,68,68,0.15)' : 'rgba(99,102,241,0.15)', border: '1px solid ' + (listening ? 'rgba(239,68,68,0.4)' : 'rgba(99,102,241,0.4)'), color: listening ? '#ef4444' : '#a5b4fc', padding: '7px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}
              >
