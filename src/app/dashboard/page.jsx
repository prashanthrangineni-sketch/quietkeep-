...
function getAISuggestions(text) {
  const t = text.toLowerCase();
  const suggestions = [];
  if (/call|ring|phone|contact/.test(t)) { suggestions.push({ icon: '\u{1F4DE}', text: 'Set as Contact type', action: 'contact' }); suggestions.push({ icon: '\u{23F0}', text: 'Add a reminder to follow up', action: 'reminder' }); }
  if (/tomorrow|tonight|morning|evening|monday|tuesday|wednesday|thursday|friday|weekend/.test(t)) { suggestions.push({ icon: '\u{23F0}', text: 'Set a reminder for this', action: 'reminder' }); }
  if (/buy|order|get|purchase|pick up/.test(t)) { suggestions.push({ icon: '\u{1F6D2}', text: 'Mark as a task to complete', action: 'task' }); }
  if (/meet|meeting|appointment|doctor|dentist|interview/.test(t)) { suggestions.push({ icon: '\u{1F4C5}', text: 'Add to reminders with time', action: 'reminder' }); }
  if (/idea|think|consider|maybe|what if/.test(t)) { suggestions.push({ icon: '\u{1F4A1}', text: 'Save as a note to revisit', action: 'note' }); }
  if (/email|send|reply|respond/.test(t)) { suggestions.push({ icon: '\u{1F4E7}', text: 'Add contact info', action: 'contact' }); }
...
export default function Dashboard() {
  ...
  // CHANGED: reads from 'keeps' table
  ...
  // CHANGED: updates 'status' in keeps
  ...
  // CHANGED: filters on 'status' not 'state'
      ...
      <div style={{ borderBottom: '1px solid #1e1e2e', padding: '10px 16px', backgroundColor: 'rgba(10,10,15,0.98)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
        ...
        {displayIntents.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 24px', border: '1px dashed #1e293b', borderRadius: '14px', color: '#334155' }}>
            <div style={{ fontSize: '32px', marginBottom: '10px' }}>{activeTab === 'open' ? '\u{1F399}' : '\u{2705}'}</div>
...
// CHANGED: reads intent_type and status (not assist_mode and state)
function IntentCard({ intent, onUpdateState, onDelete }) {
  const [expanded, setExpanded] = useState(false);
  const emoji = TYPE_EMOJI[intent.intent_type] || '\u{1F4DD}';
