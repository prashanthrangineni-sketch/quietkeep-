...
import { useFeatureGate, UpgradeScreen, FeatureErrorUI, GateLoadingUI } from '@/lib/hooks/useFeatureGate';
...
export default function HealthPage() {
  ...
  const { status: gateStatus, tier } = useFeatureGate('health_streak');
  ...
  async function getAIInsight() {
    ...
    try {
      // FIX B1: was calling Anthropic directly from browser (empty key + security risk).
      // Now routes through /api/ai/summary which holds the API key server-side.
      // auth from useAuth context
            const { data, error } = await safeFetch('/api/ai/summary', {
        method: 'POST',
        body: JSON.stringify({
          mode: 'health_insight',
          content: `User health data last 7 days:\n${summary}\n\nGive 1 short actionable health tip (2 sentences max) based on patterns. Be specific and warm.`,
        }),
        token: accessToken || '',
      });
      if (!error && data) {
        setInsight(data.summary || data.result || 'Keep logging consistently for better insights!');
      } else {
        setInsight('Log a few more days for personalized insights.');
      }
  ...
  if (loading || gateStatus === 'loading') return (<div style={{ minHeight:'100vh', background:'#0d1117', display:'flex', alignItems:'center', justifyContent:'center' }}><div style={{ color:'#6366f1' }}>Loading…</div></div>);
  if (gateStatus === 'LOCKED') return (<div style={{ minHeight:'100vh', background:'#0d1117' }}><NavbarClient /><UpgradeScreen feature="Health Log" tier={tier} /></div>);
  if (gateStatus === 'ERROR')  return (<div style={{ minHeight:'100vh', background:'#0d1117' }}><NavbarClient /><FeatureErrorUI onRetry={() => window.location.reload()} /></div>);
