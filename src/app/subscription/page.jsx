// File: src/app/subscription/page.jsx
// Razorpay subscription tiers — Sprint 4
'use client';
...
const PLANS = [
  ...
  },
  {
    ...
    label: '₹99 / month',
    color: '#6366f1',
    razorpayPlanId: 'plan_plus_monthly', // replace with actual Razorpay plan ID
  ...
  },
  {
    ...
    label: '₹199 / month',
    color: '#22c55e',
    razorpayPlanId: 'plan_family_monthly', // replace with actual Razorpay plan ID
...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return;
      setUser(user);
      supabase.from('subscriptions').select('plan_id,is_active').eq('user_id', user.id).eq('is_active', true).order('created_at', { ascending: false }).limit(1)
        .then(({ data }) => {
  ...
  async function handleUpgrade(plan) {
    ...
    // Create order via API route (to be built)
    try {
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response) => {
          ...
          await fetch('/api/razorpay/verify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...response, plan_id: plan.id, user_id: user.id }),
          });
  ...
  return (
    <>
      ...
      <div style={{ minHeight:'100vh', background:'#0a0a0f', color:'#f1f5f9', paddingBottom:'100px' }}>
        {toast && <div style={{ position:'fixed', top:'70px', left:'50%', transform:'translateX(-50%)', background:'#1e1e2e', border:'1px solid #6366f1', borderRadius:'10px', padding:'10px 20px', color:'#f1f5f9', fontSize:'14px', zIndex:9999 }}>{toast}</div>}
          ...
          {PLANS.map(plan => {
            ...
            return (
              <div key={plan.id} style={{ background: isCurrent ? 'rgba(99,102,241,0.06)' : '#0f0f1a', border: `1px solid ${isCurrent ? plan.color+'60' : '#1e1e2e'}`, borderRadius:'16px', padding:'22px', marginBottom:'14px', position:'relative' }}>
                {plan.badge && <div style={{ position:'absolute', top:'-10px', right:'18px', background:plan.color, color:'#fff', fontSize:'10px', fontWeight:700, padding:'3px 10px', borderRadius:'20px', letterSpacing:'0.08em' }}>{plan.badge}</div>}
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'14px' }}>
                  ...
                  {isCurrent && <div style={{ background:'rgba(99,102,241,0.15)', color:'#a5b4fc', padding:'4px 12px', borderRadius:'20px', fontSize:'11px', fontWeight:600 }}>CURRENT</div>}
                </div>
