...
const BILL_TYPES = [
  ...
  { value:'other',label:'Other',emoji:'📋',color:'var(--text-muted)',dueDay:1 },
];
...
export default function BillsPage() {
  ...
  const inp = { width:'100%', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, color:'var(--text)', padding:'11px 14px', fontSize:14, fontFamily:'inherit', outline:'none', boxSizing:'border-box' };
      ...
      <div style={{ maxWidth:480, margin:'0 auto', padding:'6rem 16px 6rem' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          <div>
            ...
            <div style={{ fontSize:13, color:'var(--text-muted)' }}>Tax · FASTag · Electricity · EMIs</div>
          </div>
        ...
        {loading ? <div style={{ textAlign:'center', padding:'40px', color:'var(--text-muted)' }}>Loading…</div>
          : bills.length===0 ? (
            <div style={{ textAlign:'center', padding:'40px 20px', color:'var(--text-subtle)', fontSize:13 }}>
            ...
            return (
              <div key={bill.id} style={{ background:'var(--surface)', border:`1px solid ${urgent?'rgba(239,68,68,0.3)':soon?'rgba(245,158,11,0.25)':'var(--surface)'}`, borderRadius:12, padding:'14px 16px', marginBottom:8, display:'flex', alignItems:'center', gap:12 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap', fontSize:12, color:'var(--text-muted)', marginTop:3 }}>
                  ...
                  {bill.last_paid_at&&<div style={{ fontSize:11, color:'var(--text-subtle)', marginTop:2 }}>Paid: {bill.last_paid_at}</div>}
                </div>
