...
import { readBrief } from '@/components/VoiceTalkback';
...
export default function DailyBriefPage() {
  ...
  async function loadBrief(uid) {
    ...
    const [
      ...
      supabase.from('trip_plans').select('destination,travel_date,return_date').eq('user_id', uid).gte('travel_date', today).order('travel_date').limit(3),
  ...
  async function generateAISummary() {
    ...
    try {
      const langCode = displayLocale || 'en';
      const { data: res, error: resErr } = await apiPost('/api/daily-brief-summary', { brief, userLanguage: langCode }, accessToken);
      const data = res;
      if (data.error) throw new Error(data.error);
      setAiSummary(data.summary || 'Could not generate summary. Please try again.');
    } catch (e) {
      const msg = (e?.message || '').toLowerCase();
      if (msg.includes('ai not configured') || msg.includes('anthropic')) {
        setAiSummary('AI not configured — contact support.');
      } else if (msg.includes('limit') || msg.includes('upgrade')) {
        setAiSummary('Daily AI limit reached. Upgrade your plan at quietkeep.com/pricing.');
      } else if (msg.includes('unauthorized') || msg.includes('session')) {
        setAiSummary('Session expired. Please refresh the page.');
      } else {
        setAiSummary('Error generating summary. Check your connection and try again.');
      }
    }
    setLoadingAI(false);
  }
  ...
  const Section = ({ title, children }) => (
    <div className="qk-card" style={{ padding: 16, marginBottom: 14 }}>
      <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-subtle)', marginBottom: 12 }}>{title}</div>
      {children}
    </div>
  );
      ...
      <div className="qk-container">
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            ...
            <button
              onClick={() => readBrief(aiSummary || 'No summary available yet. Tap Generate Summary below.')}
        ...
        <div style={{ background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 12, padding: 18, marginBottom: 14 }}>
          ...
          {aiSummary ? (
            ...
            <div style={{ color: 'var(--text-subtle)', fontSize: 13, marginBottom: 12 }}>Generate a natural language summary of your day with Claude.</div>
          )}
          <button
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 4 }}>{n.body}</div>
                  ...
                  <button onClick={e => { e.stopPropagation(); dismissNudge(n.id); }} style={{ background: 'none', border: 'none', color: 'var(--text-subtle)', cursor: 'pointer', fontSize: 16, marginLeft: 8 }}>✕</button>
                </div>
              </div>
            ))}
          </Section>
        ...
        {brief?.trips?.length > 0 && (
          <Section title="Upcoming Trips">
            {brief.trips.map((t, i) => <Item key={i} text={t.destination} meta={`${t.travel_date} → ${t.return_date || '?'}`} />)}
          </Section>
        )}
        ...
        <div className="qk-card" style={{ padding: 16, marginBottom: 0 }}>
          <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-subtle)', marginBottom: 12 }}>Share Today&apos;s Brief</div>
