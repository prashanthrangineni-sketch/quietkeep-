...
export default function SubscriptionComponent() {
  ...
  return (
    <div style={{ marginTop: '32px' }}>
      ...
      {subscriptions.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '24px' }}>
          <div style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>Monthly Recurring</div>
            <div style={{ fontSize: '18px', fontWeight: '700', color: '#6366f1' }}>₹{monthlyTotal.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</div>
          </div>
          <div style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>Annual Recurring</div>
              ...
              <div key={sub.id} style={{ backgroundColor: 'var(--surface)', border: isOverdue ? '1px solid #ef4444' : isDueSoon ? '1px solid #f59e0b' : '1px solid #1e293b', borderRadius: '12px', padding: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--text-subtle)', marginTop: '2px' }}>{cycleConfig.label}</div>
                  </div>
                  <button
                    onClick={() => handleDeleteSubscription(sub.id)}
                    style={{ backgroundColor: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: '11px', cursor: 'pointer' }}
                  >
                    Delete
                  </button>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginBottom: '6px' }}>
                  <div style={{ color: 'var(--text-muted)' }}>₹{sub.amount.toLocaleString('en-IN')}/{cycleConfig.value}</div>
      ...
      {subscriptions.length === 0 && (
        <div style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px', textAlign: 'center', marginBottom: '24px' }}>
          <div style={{ fontSize: '24px', marginBottom: '12px' }}>📺</div>
          <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>No subscriptions yet</div>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px' }}>Netflix, Spotify, gym, etc.</div>
        </div>
      )}
      ...
      {showForm && (
        <div style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '14px', padding: '20px' }}>
          <h3 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text)', margin: '0 0 16px' }}>New Subscription</h3>
