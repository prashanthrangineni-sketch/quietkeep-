...
else if (/\bnoon\b|\bmidday\b/.test(t)) { hours = 12; minutes = 0; } if (hours < 7) hours += 12;
}
}
...
recognition.onresult = (e) => { const transcript = Array.from(e.results).map(r => r[0].transcript).join('');
...
.from('intents').select('*').eq('user_id', uid)
...
user_id: user.id, content: content.trim(), state: 'open',
assist_mode: assistMode, contact_info: contactInfo || null,
remind_at: remindAt || null, intent_type: assistMode,
intent_status: 'captured', parsing_method: 'manual',
metadata: remindAt ? { reminder_type: reminderType } : null,
}]);
if (!error) {
await supabase.from('audit_log').insert([{
user_id: user.id, action: 'intent_created', service: 'dashboard',
details: { content: content.trim().substring(0, 100), intent_type: assistMode,
has_reminder: !!remindAt, reminder_type: remindAt ? reminderType : null },
}]);
...
state, ...(state === 'closed' ? { completed_at: new Date().toISOString() } : {}),
}).eq('id', id);
await supabase.from('audit_log').insert([{
...
<div style={{ width: '40px', height: '40px', border: '3px solid #6366f1',
...
{toast && (
<div style={{ position: 'fixed', top: '70px', left: '50%',
transform: 'translateX(-50%)', backgroundColor: '#1e1e2e',
border: '1px solid #6366f1', borderRadius: '10px',
...
{/* Sub-nav */}
<div style={{ borderBottom: '1px solid #1e1e2e', padding: '10px 16px',
backgroundColor: 'rgba(10,10,15,0.98)', display: 'flex',
...
<a href="/calendar" style={{ color: '#94a3b8', textDecoration: 'none',
fontSize: '11px', padding: '5px 10px', border: '1px solid #1e293b',
borderRadius: '6px', whiteSpace: 'nowrap' }}>Calendar</a>
<a href="/daily-brief" style={{ color: '#94a3b8', textDecoration: 'none',
fontSize: '11px', padding: '5px 10px', border: '1px solid #1e293b',
borderRadius: '6px', whiteSpace: 'nowrap' }}>Brief</a>
<a href="/finance" style={{ color: '#94a3b8', textDecoration: 'none',
fontSize: '11px', padding: '5px 10px', border: '1px solid #1e293b',
borderRadius: '6px', whiteSpace: 'nowrap' }}>Finance</a>
<a href="/settings" style={{ color: '#94a3b8', textDecoration: 'none',
fontSize: '11px', padding: '5px 10px', border: '1px solid #1e293b',
borderRadius: '6px', whiteSpace: 'nowrap' }}>Settings</a> <a href="/profile" style={{ color: '#94a3b8', textDecoration: 'none',
fontSize: '11px', padding: '5px 10px', border: '1px solid #1e293b',
...
fontSize: '11px', cursor: 'pointer', whiteSpace: 'nowrap' }}>Sign Out</button>
</div>
</div>
...
<div key={i} style={{ backgroundColor: '#0f0f1a', border: '1px solid #1e1e2e',
...
{listening && (
<div style={{ display: 'flex', alignItems: 'center', gap: '8px',
marginBottom: '10px', backgroundColor: 'rgba(239,68,68,0.08)',
border: '1px solid rgba(239,68,68,0.2)', borderRadius: '8px', padding: '8px 12px' }}>
<span style={{ width: '8px', height: '8px', backgroundColor: '#ef4444',
borderRadius: '50%', display: 'inline-block',
animation: 'pulse 1s ease infinite' }} />
<span style={{ fontSize: '12px', color: '#ef4444' }}>
Listening... say date/time naturally e.g. "tomorrow 3pm"
</span>
</div>
)}
{autoDetected && (
<div style={{ display: 'flex', alignItems: 'center', gap: '8px',
marginBottom: '10px', backgroundColor: 'rgba(99,102,241,0.08)',
border: '1px solid rgba(99,102,241,0.3)', borderRadius: '8px', padding: '8px 12px' }}>
...
rows={3}
style={{ width: '100%', backgroundColor: '#0a0a0f',
...
style={{ width: '100%',
backgroundColor: remindAt ? 'rgba(99,102,241,0.05)' : '#0a0a0f',
border: '1px solid ' + (remindAt ? '#6366f150' : '#1e293b'),
borderRadius: '8px', padding: '8px', color: '#f1f5f9',
...
<label style={{ fontSize: '11px', color: '#475569',
display: 'block', marginBottom: '8px' }}>How to remind you?</label>
<div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
{[
{ value: 'app', label: 'App' },
...
<button key={opt.value} onClick={() => setReminderType(opt.value)}
style={{ padding: '7px 12px', borderRadius: '8px', cursor: 'pointer',
border: '1px solid ' + (reminderType === opt.value ? '#6366f1' : '#1e293b'),
backgroundColor: reminderType === opt.value
? 'rgba(99,102,241,0.15)' : '#0a0a0f',
...
padding: '6px 10px', backgroundColor: 'rgba(245,158,11,0.08)',
...
style={{ width: '100%', marginTop: '10px', backgroundColor: '#0a0a0f',
border: '1px solid #1e293b', borderRadius: '8px', padding: '9px 12px',
color: '#f1f5f9', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }} />
)}
<div style={{ display: 'flex', alignItems: 'center',
...
style={{ width: '100%', backgroundColor: '#0f0f1a',
...
color: '#475569', cursor: 'pointer', fontSize: '16px' }}>x</button>
)}
</div>
{/* Tabs */}
<div style={{ display: 'flex', gap: '4px', marginBottom: '14px',
backgroundColor: '#0f0f1a', padding: '4px', borderRadius: '10px',
border: '1px solid #1e1e2e' }}>
{[{ key: 'open', label: 'Open (' + openIntents.length + ')' },
{ key: 'closed', label: 'Done (' + closedIntents.length + ')' }].map(tab => (
...
fontSize: '13px', fontWeight: '600' }}>{tab.label}</button>
))}
</div>
{/* Keeps list */}
{displayIntents.length === 0 ? (
<div style={{ textAlign: 'center', padding: '40px 24px',
border: '1px dashed #1e293b', borderRadius: '14px', color: '#334155' }}>
{searchQuery
? <><div style={{ fontSize: '32px', marginBottom: '10px' }}>?</div>
<div>No keeps matching "{searchQuery}"</div></>
: activeTab === 'open'
? <><div style={{ fontSize: '32px', marginBottom: '10px' }}>?</div>
<div>Tap Voice or type to add your first keep</div></>
: <><div style={{ fontSize: '32px', marginBottom: '10px' }}>?</div>
<div>No completed keeps yet</div></>
}
</div>
) : (
...
onUpdateState={updateState} onDelete={handleDelete} />
))} </div>
)}
</div>
<style>{`
...
const reminderIcons = {
app: '\u{1F4F3}', alarm: '\u{23F0}', whatsapp: '\u{1F4AC}', email: '\u{1F4E7}'
};
const reminderType = intent.metadata?.reminder_type;
return (
<div style={{ backgroundColor: '#0f0f1a',
border: '1px solid ' + (isClosed ? '#1a1a2e' : '#1e1e2e'),
...
<p style={{ margin: '0 0 8px', fontSize: '15px', color: '#e2e8f0',
lineHeight: 1.5, textDecoration: isClosed ? 'line-through' : 'none' }}>
{intent.content}
</p>
<div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
<span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '100px',
fontWeight: '700', backgroundColor: color + '18', color,
textTransform: 'uppercase' }}>{intent.state}</span>
{intent.remind_at && (
<span style={{ fontSize: '11px', color: '#64748b' }}>
{reminderIcons[reminderType] || '\u{23F0}'}{' '}
{new Date(intent.remind_at).toLocaleString('en-IN',
{ day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
</span>
)}
{intent.contact_info && (
<span style={{ fontSize: '11px', color: '#64748b' }}>
{'\u{1F4DE}'} {intent.contact_info}
</span>
)}
<span style={{ fontSize: '11px', color: '#1e293b', marginLeft: 'auto' }}>
{new Date(intent.created_at).toLocaleDateString('en-IN',
...
paddingTop: '10px', borderTop: '1px solid #1a1a2e', flexWrap: 'wrap' }}>
<button onClick={() => onUpdateState(intent.id, 'closed')}
style={{ backgroundColor: '#052010', border: '1px solid #166534',
color: '#22c55e', padding: '5px 12px', borderRadius: '6px',
fontSize: '12px', cursor: 'pointer', fontWeight: '600' }}>Done</button>
<button onClick={() => setExpanded(e => !e)}
style={{ backgroundColor: 'transparent', border: '1px solid #1e293b',
color: '#64748b', padding: '5px 12px', borderRadius: '6px',
fontSize: '12px', cursor: 'pointer' }}>{expanded ? 'Less' : 'More'}</button>
<button onClick={() => onDelete(intent.id)}
style={{ backgroundColor: 'transparent', border: '1px solid #2d1515',
color: '#ef4444', padding: '5px 12px', borderRadius: '6px',
fontSize: '12px', cursor: 'pointer', marginLeft: 'auto' }}>Delete</button>
</div>
)}
{expanded && (
<div style={{ marginTop: '10px', paddingTop: '10px',
borderTop: '1px solid #1a1a2e', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
...
backgroundColor: STATE_COLOR[s] + '15',
border: '1px solid ' + STATE_COLOR[s] + '40',
color: STATE_COLOR[s], padding: '4px 10px', borderRadius: '6px',
fontSize: '11px', cursor: 'pointer', fontWeight: '600' }}>{s}</button>
))}
</div>
)}
</div>
